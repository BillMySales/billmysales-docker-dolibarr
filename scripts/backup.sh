#!/bin/bash
# Backups of the database and Dolibarr's data.
#
...
# (documents, htdocs/conf and htdocs/custom; the code comes from the image),
...
BACKUP_DIR=/backups
WWW_DIR=/var/www
FILES="documents dolibarr/htdocs/conf dolibarr/htdocs/custom"
...
backup() {
    ...
    tar -C "${WWW_DIR}" -czf "${tmp}-files.tar.gz" ${FILES}
...
restore() {
    ...
    echo "==> Restoring documents, conf and custom from ${files}"
    local dir
    for dir in ${FILES}; do
        find "${WWW_DIR}/${dir}" -mindepth 1 -maxdepth 1 -exec rm -rf {} +
    done
    tar -C "${WWW_DIR}" -xzpf "${files}"
    echo "==> Restored ${ts}"
}
